using Test
using ChunkedCSV: ChunkedCSV, BufferedVector, skip_element!
...
@testset "BufferedVectors" begin
    ...
    bv = BufferedVector{Int}()
    push!(bv, 1)
    @test length(bv.elements) == ChunkedCSV.BufferedVectors._grow_by(Int)
    @assert ChunkedCSV.BufferedVectors._grow_by(Int) > 0
    @test_throws BoundsError bv[2]
    ChunkedCSV.unsafe_push!(bv, 42)
    @test bv == [1, 42]

    code = """
    using ChunkedCSV: BufferedVector
    using Test
    ...
    push!(bv, Int32(1))
    @test length(bv.elements) == ChunkedCSV.BufferedVectors._grow_by(Int32)
    @assert ChunkedCSV.BufferedVectors._grow_by(Int32) > 0

    bv = BufferedVector{Int}([1,2,3], 3)
    ChunkedCSV.shiftleft!(bv, 1)
    @test bv == [2,3]

    bv = BufferedVector{Int}([1,2,3], 3)
    ChunkedCSV.shiftleft!(bv, 2)
    @test bv == [3]

    bv = BufferedVector{Int}([1,2,3], 3)
    ChunkedCSV.shiftleft!(bv, 3)
    @test bv == []

    bv = BufferedVector{Int}([1,2,3], 3)
    ChunkedCSV.shiftleft!(bv, 4)
    @test bv == []

    @test_throws ArgumentError ChunkedCSV.shiftleft!(bv, -1)
end