...
function noop_stream(x::String)
    (_, io) = mktemp()
    write(io, x)
    seekstart(io)
    return NoopStream(io)
end
...
function gzip_stream_autodetect(x::String)
    (path, io) = mktemp()
    write(io, GzipCompressorStream(IOBuffer(x)))
    close(io)
    return path
...
for io_t in (IOBuffer, noop_stream, iostream, gzip_stream, gzip_stream_autodetect)
    @testset "Simple file parsing ($(io_t))" begin
    ...
    @testset "Empty input ($(io_t))" begin
        for alg in [:serial, :singlebuffer, :doublebuffer]
    ...
    @testset "default_colname_prefix ($(io_t))" begin
        testctx = TestContext()
        parse_file(io_t("""
            1,2
            3,4
    ...
    @testset "escaped quote ($(io_t))" begin
        testctx = TestContext()
        parse_file(io_t("""
            a|b
            1|"2\\""
    ...
    @testset "Opening and closing quotes ($(io_t))" begin
        testctx = TestContext()
        parse_file(io_t("""
            a,b
            1,S2E
    ...
    @testset "Two escape characters before closing quote ($(io_t))" begin
        for alg in [:serial, :singlebuffer, :doublebuffer]
            @testset "$alg" begin
                testctx = TestContext()
                parse_file(io_t("""
    ...
    for buffersize in (12, 13, 14, 15)
        @testset "Ending on an escapechar @ $buffersize ($(io_t))" begin
    ...
    @testset "Carriage return ($(io_t))" begin
        @testset "ints" begin
            testctx = TestContext()
            parse_file(io_t("""
                a,b
    ...
    @testset "Quoted fields ($(io_t))" begin
    ...
    @testset "Each chunk ends on a quote ($(io_t))" begin
        testctx = TestContext()
        parse_file(io_t("""
            0,"S\"\"\"
            1,"S\"\"\"
    ...
    @testset "custom newlinechar" begin
        for alg in [:serial, :singlebuffer, :doublebuffer]
            @testset "$alg int" begin
                testctx = TestContext()
    ...
    @testset "Ending on an newline ($(io_t))" begin
        testctx = TestContext()
        parse_file(io_t("""
            a,b
            0,z
